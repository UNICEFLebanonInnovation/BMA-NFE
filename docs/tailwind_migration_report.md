...
6. Run `scripts/build_tailwind.sh` (runs `tailwind install` then `tailwind build`) to bootstrap npm dependencies and generate production CSS. The theme currently uses `tailwindcss@1.9.6` and invokes `node ./node_modules/tailwindcss/lib/cli.js build ...` (v1 CLI syntax, avoiding shell PATH/.bin resolution issues) for compatibility with older npm/node environments where `npx` is unavailable.
