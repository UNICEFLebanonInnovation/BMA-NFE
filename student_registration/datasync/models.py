...
class SyncedRecord(TimeStampedModel):
    ...
    pending_links = JSONField(
        default=dict, blank=True,
        verbose_name=_('References to resolve when their target arrives'),
        help_text=_(
            'Natural keys the last push carried that pointed at records not '
            'replicated yet. The target\'s own handler consults these on arrival.'
        ),
    )
